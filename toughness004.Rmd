...
```{r}
plots_shear_area <- function(id,Ai, Bi, Ci, Di) {
  tim_cvn %>%
    filter(.data$id == .env$id) %>%
...
data %>%
  ...
  labs(title = paste("ID =", ids[idx], "Final Fit"),
        x="Temperature (°F)", 
       y = "Shear Area (%)")
...
make_plot2(ids[idx],coefs[1], coefs[2],coefs[3], coefs[4])
```
...
make_plot3(ids[idx],coefs[1],coefs[2],coefs[3],coefs[4])
```
